...
dir.create("3 ML", recursive = TRUE, showWarnings = FALSE)

train_genematrix <- qs_read("2 MLlist and data/train_genematrix.qs",nthreads = 4)
test_genematrix <- qs_read("2 MLlist and data/test_genematrix.qs",nthreads = 4)

X <- as.matrix(train_genematrix[,setdiff(colnames(train_genematrix), "Integrated_Score")])
Y <- as.matrix(train_genematrix$Integrated_Score)

X_train <- as.matrix(train_genematrix[,setdiff(colnames(train_genematrix), "Integrated_Score")])
X_test <- as.matrix(test_genematrix[,setdiff(colnames(test_genematrix), "Integrated_Score")])
...
rf_metrics <- bind_rows(
  data.frame(
    Set = "Train",
    eval_regression(Y_train_rf, pred_train_rf)
  ),
  data.frame(
    Set = "test",
    eval_regression(Y_test_rf, pred_test_rf)
  )
)
...
write.csv(rf_importance,file = "3 ML/rf_importance-new.csv")
...
plot_fit_scatter(Y_train_rf, pred_train_rf, n_plot = 3000, title = "Training set")
plot_fit_scatter(Y_test_rf, pred_test_rf, n_plot = 3000, title = "Test set")
...
lgb_importance$Feature <- normalize_feature(lgb_importance$Feature)

write.csv(lgb_importance,file = "3 ML/lgb_importance-new.csv")
...
write.csv(xgb_importance,file = "3 ML/xgb_importance-new.csv")
